...
def calculate_ecom_runway(product_url: str, monthly_subs: float):
    ...
    if current_status == "CRITICAL UNDERCUT":
        ai_recommendation = f"Risk Alert: Competitor is undercutting at ${competitor_price}. Drop target price to ${our_suggested_price} to recapture traffic and secure your ${monthly_subs} runway."
    else:
        ai_recommendation = f"System Healthy: Margins stable. Hold price around ${our_suggested_price}. Sell {units_to_cover_subs} units to clear your software tools overhead."
...
def premium_dashboard():
    ...
    <head>
        ...
        <style>
            body { font-family: sans-serif; background-color: #0b0f19; color: #f8fafc; margin: 0; padding: 20px; display: flex; justify-content: center; align-items: center; min-height: 100vh; }
            .card { background: #1e293b; border: 1px solid #334155; padding: 35px; border-radius: 24px; width: 100%; max-width: 500px; box-shadow: 0 20px 40px rgba(0,0,0,0.5); }
            ...
            label { display: block; font-size: 0.85rem; font-weight: 700; color: #cbd5e1; text-transform: uppercase; margin-bottom: 10px; }
            ...
            .results-box { margin-top: 35px; background: #060911; border: 1px solid #1e293b; border-radius: 20px; padding: 24px; display: none; }
    ...
    <body>
        <div class="card">
            ...
            <div class="results-box" id="results">
                <div class="status-line">
                    <span style="font-size: 0.85rem; color:#64748b; font-weight:700; text-transform:uppercase;">AI Verdict:</span>
        ...
        <script>
            function runOptimization(){
                ...
                if(!url || !spend){ alert("Please fill in all blocks!"); return; }
