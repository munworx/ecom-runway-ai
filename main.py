...
def enterprise_command_suite():
    # Complete text template isolation - 100% immune to bracket errors
    html_layout = [
        ...
        "<div class='util-row'><button class='util-btn' onclick='copyData()'>Copy JSON</button><button class='util-btn' onclick='downloadCSV()'>Download CSV</button></div></div></div>",
        ...
        "document.getElementById('resPlan').innerHTML=d.action_plan;document.getElementById('results').style.display='block';});}function copyData(){if(!currentPayload)return;navigator.clipboard.writeText(JSON.stringify(currentPayload,null,2));alert('Metrics Copied!');}",
        "function downloadCSV(){if(!currentPayload)return;let c='Metric,Value\\n';Object.keys(currentPayload).forEach(k=>{c+=k+',\"'+currentPayload[k]+'\"\\n';});const e=encodeURI('data:text/csv;charset=utf-8,'+c);",
