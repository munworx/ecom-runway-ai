...
def enterprise_command_suite():
    # Sourcing a premium, bulletproof data terminal interface template externally
    html_template = """
    <!DOCTYPE html>
    <html lang="en">
    <head>
        ...
        <style>
            ...
            .card { background: #1e293b; border: 1px solid #334155; padding: 35px; border-radius: 24px; width: 100%; max-width: 540px; box-shadow: 0 40px 80px rgba(0,0,0,0.5); }
            .header-zone { text-align: center; margin-bottom: 25px; border-bottom: 1px dashed #334155; padding-bottom: 20px; }
            h1 { color: #38bdf8; font-size: 2.2rem; margin: 0 0 6px 0; }
            .subtitle { color: #94a3b8; font-size: 0.95rem; }
            .form-group { margin-bottom: 18px; }
            label { display: block; font-size: 0.8rem; font-weight: 700; color: #cbd5e1; text-transform: uppercase; margin-bottom: 8px; }
            ...
            button { width: 100%; padding: 16px; background: #0ea5e9; color: white; border: none; border-radius: 14px; font-size: 1.05rem; font-weight: 700; cursor: pointer; margin-top: 10px; }
    ...
    <body>
        <div class="card">
            <div class="header-zone">
                ...
                <div class="subtitle">Enterprise Data Command Suite & Pricing Radar</div>
            </div>
            ...
            <button onclick="runOptimization()">Execute Enterprise Intelligence</button>
            <div class="results-box" id="results">
                <div class="status-line">
                    <span style="font-size: 0.85rem; color:#64748b; font-weight:700; text-transform:uppercase;">Logs: <span style="color:#34d399">ONLINE</span></span>
                ...
                <div class="grid-3">
                    ...
                    <div class="stat"><div class="stat-val" id="resUnits">0</div><div class="stat-lbl">Target Units</div></div>
                </div>
                <div class="section-title"><span>Unit Economics Layer</span></div>
                <div class="grid-3">
                    <div class="stat"><div class="stat-val" id="resCOGS">$0.00</div><div class="stat-lbl">Est. Unit Cost</div></div>
                    <div class="stat"><div class="stat-val" id="resMargin">$0.00</div><div class="stat-lbl">Net Margin</div></div>
                    <div class="stat"><div class="stat-val" id="resAdSpend">$0.00</div><div class="stat-lbl">Ad Overhead</div></div>
                </div>
                <div class="progress-container">
                    <div style="display:flex; justify-content:space-between; font-size:0.75rem; color:#94a3b8; font-weight:600;"><span>RUNWAY SECURITY RADAR</span><span id="progressText">0%</span></div>
                    <div class="progress-bar"><div class="progress-fill" id="pFill"></div></div>
                </div>
                <div class="section-title"><span>AI Recommendation Blueprint</span></div>
                <div class="advice" id="resPlan">Processing...</div>
                <div class="util-row">
                    <button class="util-btn" onclick="copyData()">Copy JSON</button>
                    <button class="util-btn" onclick="downloadCSV()">Download CSV</button>
                </div>
            </div>
        </div>
        <script>
            ...
            function runOptimization(){
                const u = document.getElementById("prodUrl").value;
                const s = document.getElementById("subSpend").value;
                if(!u || !s){ alert("Complete variables!"); return; }
                fetch("/analyze-runway?product_url=" + encodeURIComponent(u) + "&monthly_subscription_spend=" + s)
                    .then(res => res.json())
                    .then(w => {
                        currentPayload = w.payload;
                        const d = w.payload;
                        document.getElementById("resStatus").innerText = d.status;
                        document.getElementById("resStatus").style.backgroundColor = d.status === "HEALTHY MARGIN" ? "#059669" : "#d97706";
                        document.getElementById("resTrend").innerText = d.market_trend;
